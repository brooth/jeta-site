...
*Dependency injection* is the most powerful part of *Jeta*. In addition to the common `DI` functions it offers some extra:
...
Before we start, let's define a helper method we're goint to inject with:
...
Well, the simplest way to inject an entity - define this entity as *MetaEntity*. Each time the `inject(this)` method is invoked, a new instance of *Producer* is injected. If you need a single instance of an entity, set up this flag: `@MetaEntity(singleton=true)`. <span class="label label-info">Note</span> It makes a singleton per a `Scope`. Read below about the [scopes](#Scopes) .
...
<br/>
Besides of straight injection, you can provide it using `Lazy`, `Provider` or inject its `class`:
...
<span class="label label-info">Note</span> Ones you defined a `MetaFactory`, you can use it for the all dependencies of a master, including entities without parameters, provided via `Lazy`, `Provider` and `Class`.


<div class="alert alert-success" role="alert">
Note that the injection expression must match to the entity construction, ie. you can't inject an entity without meta factory if there is no empty constructor for, otherwise the code won't be compiled. This aspect makes <code>Jeta DI</code> more helpful for development. If the code is compiled it should work properly.
</div>


### MetaEntity Provider
In case you need a provider of an entity, eg. to be able to inject thirdparty classes, you need to annotate this provider with `@MetaEntity(of=Producer.class)`:
...
<span class="label label-info">Note</span> In case of entity provider you must explicitly define factory methods via `@Constructor` annotation. Also, `staticConstructor` is used to create the provider, not entities. If it's needed to create entities via static factory method, add `@Constructor` annotation on it:
...
You can define as many scopes as needed. However, at lease one *Scope* must be in order to inject module's meta entities. So, it's important to understand that no one entity can be provided outside a scope. In the other hand, one entity can belong to many scopes.
...
Now, you can access to the `Application` instance from a meta entity:
...
*Jeta* brings all the needed annotations to inject right away. You can find these in `org.brooth.jeta.inject` package. Nevertheless, it's allowed to use thirdparty annotations as the aliases. It might be useful for the projects that already use other DI frameworks.

Let's say we want to use `javax.inject.Inject` to supply meta entities. To do so, modify your `jeta.properties`:
...
<span class="label label-warning">Pay attension</span> To let *InjectController (StaticInjectController)* now about `javax.inject.Inject` you also have to pass this annotation to the constructors, otherwise the controllers won't find the masters that use there annotations.
...
First, we need to create a *Module* and *Scope*:
...
As noticed above, *Jeta DI* lets you to extend meta entities easily. For example, we'll modify the *Hello, World!* example above. Instead of *"Hello, World!"* let's inject *"Hello, Universe!"*.
